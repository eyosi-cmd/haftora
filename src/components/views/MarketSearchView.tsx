...
import { Search, Database, RefreshCw, X, ArrowUpRight, ArrowDownRight, Layers, DollarSign, Building2, TrendingUp, Check, ExternalLink, RotateCcw, Clock, Sparkles } from 'lucide-react';
import { runAIMarketScreener, AIScreenerResult } from '../../services/aiScreener';
...
export const MarketSearchView: React.FC = () => {
  ...
  // ── AI Screener Assistant State ─────────────────────────────────────
  const [showAIAssistant, setShowAIAssistant] = useState(false);
  const [aiPrompt, setAiPrompt]               = useState('');
  const [aiLoading, setAiLoading]             = useState(false);
  const [aiResult, setAiResult]               = useState<AIScreenerResult | null>(null);

  const handleRunAIScreener = async (promptText: string) => {
    if (!promptText.trim()) return;
    setAiPrompt(promptText);
    setAiLoading(true);
    setShowAIAssistant(true);
    const result = await runAIMarketScreener(promptText);
    setAiResult(result);
    setAiLoading(false);
  };
  ...
  return (
    <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
        ...
        <button
          id="btn-ai-screener-toggle"
          onClick={() => {
            if (!showAIAssistant && !aiResult) {
              handleRunAIScreener('Top High-Yield Dividend ETFs');
            } else {
        ...
        >
          <Sparkles size={16} />
          {showAIAssistant ? 'Hide AI Screener' : 'Ask AI Market Screener'}
        </button>
      </div>

      {/* ── AI Screener Assistant Modal / Expanded Panel ─────────────────── */}
      {showAIAssistant && (
        <div className="card animate-in" style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E293B 100%)', color: 'white', padding: '1.4rem', borderRadius: 20, border: '1.5px solid #38BDF8', boxShadow: '0 10px 30px rgba(15,23,42,0.4)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ background: 'rgba(56,189,248,0.2)', color: '#38BDF8', borderRadius: '50%', padding: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Sparkles size={18} />
              </div>
              <div>
                <h3 style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: '1.1rem', color: '#F8FAFC' }}>
                  AI Market Screener & Analysis Assistant
                </h3>
                <span style={{ fontSize: '0.72rem', color: '#94A3B8' }}>
                  Scanning 13,000+ U.S. Stocks & ETFs in real-time
                </span>
              </div>
            </div>
            <button
              onClick={() => setShowAIAssistant(false)}
              style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: '50%', width: 26, height: 26, color: '#94A3B8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              <X size={14} />
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            <input
              type="text"
              placeholder="Ask AI (e.g. 'Find top dividend ETFs', 'Cheapest S&P 500 ETF', 'High growth AI stocks')…"
              value={aiPrompt}
              onChange={e => setAiPrompt(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter') handleRunAIScreener(aiPrompt); }}
              style={{ flex: 1, background: 'rgba(15,23,42,0.8)', border: '1px solid #334155', borderRadius: 999, padding: '0.55rem 1rem', color: 'white', fontSize: '0.85rem' }}
            />
            <button
              onClick={() => handleRunAIScreener(aiPrompt)}
              disabled={aiLoading}
              style={{ background: '#38BDF8', color: '#0F172A', border: 'none', borderRadius: 999, padding: '0.55rem 1.1rem', fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer' }}
            >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
            {[
              ...
              'Cheapest S&P 500 ETF (VOO vs IVV)',
              'High-Growth Tech & AI Stocks',
              'Safe Retirement Bond Funds'
            ].map(promptChip => (
              <button
                key={promptChip}
                onClick={() => handleRunAIScreener(promptChip)}
                style={{
          ...
          {aiLoading ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: '#38BDF8' }}>
              ...
              <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>Analyzing 13,000+ Tickers & Live Market Quotes…</div>
            </div>
          ) : aiResult ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: '0.9rem 1rem', border: '1px solid rgba(255,255,255,0.08)' }}>
                <p style={{ fontSize: '0.85rem', color: '#E2E8F0', lineHeight: 1.6, marginBottom: 8 }}>
                  {aiResult.summary}
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {aiResult.highlights.map((h, i) => (
                    <div key={i} style={{ fontSize: '0.78rem', color: '#94A3B8', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {/* Recommended Ticker Cards */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '0.75rem' }}>
                {aiResult.recommendations.map(rec => (
                  <div
                    key={rec.ticker}
                    onClick={() => { setQuery(rec.ticker); setInstrumentType('All'); setShowAIAssistant(false); }}
                    style={{ background: 'rgba(30,41,59,0.9)', border: '1px solid #334155', borderRadius: 14, padding: '0.85rem', cursor: 'pointer', transition: 'all 0.15s ease' }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, color: '#38BDF8', fontSize: '1rem' }}>
                        {rec.ticker}
                      </span>
                      {rec.price !== undefined && (
                        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, fontSize: '0.85rem', color: '#F8FAFC' }}>
                          ${rec.price.toFixed(2)}
                        </span>
                      )}
                    </div>
                    <p style={{ fontSize: '0.72rem', color: '#94A3B8', lineHeight: 1.4, marginBottom: 6, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {rec.reasoning}
                    </p>
                    <span style={{ fontSize: '0.67rem', color: '#38BDF8', fontWeight: 700 }}>
                      Inspect Ticker →
                    </span>
                  </div>
                ))}
